...
add column if not exists color_key text not null default 'blue';

alter table public.recurring_items
drop constraint if exists recurring_items_color_key_check;

alter table public.recurring_items
add constraint recurring_items_color_key_check check (
