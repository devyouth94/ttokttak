...
alter table public.recurring_items
alter column color_hex set default '#F5A3A3',
alter column color_hex set not null;

alter table public.recurring_items
add constraint recurring_items_color_hex_check check (
  color_hex ~ '^#[0-9A-F]{6}$'
);
...
create function public.create_recurring_item_with_initial_version(
  p_user_id uuid,
...
create function public.update_recurring_item_with_edit_policy(
  p_item_id uuid,
  p_user_id uuid,
  p_title_ciphertext text,
  p_description_ciphertext text,
