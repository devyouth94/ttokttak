...
type AppleRevocationResult = "identity_mismatch" | "revoke_failed" | "success";

export function isAppleAccount(user: User): boolean {
...
export async function revokeAppleAccountToken({
  authorizationCode,
  user,
}: {
  ...
  return (await revokeAppleToken(exchangeResult)) ? "success" : "revoke_failed";
}
...
function isMatchingAppleSubject({
  ...
  return getAppleIdentitySubjects(user).includes(appleSubject);
}
