...
여러 구현 이슈가 하나의 PRD 이슈로 강하게 결합되어 있으면 사용자 확인 후 하나의 기능 브랜치에서 진행할 수 있다.

이 경우 브랜치 이름에는 구현 이슈 번호 대신 PRD 이슈 번호를 사용한다.
...
새 이슈 브랜치를 만들기 전에 원격 상태를 확인한다.

기본 브랜치에 원격 변경사항을 병합하거나 rebase해야 하면 사용자에게 확인한다.
...
여러 구현 이슈를 하나의 PRD 기능 브랜치에서 진행한 경우 PR 본문에는 실제로 닫을 구현 이슈를 모두 `Closes #<issue-number>` 형식으로 포함한다.
