...
**Product working title:** Personal Recurring Reminder App

**One-line definition:**  
반복되는 생활 항목을 등록하고 알림을 받아 실행을 돕는 개인 리마인더 앱.
...
- 사용자는 목록 화면에서 등록된 리마인더를 한 번에 확인할 수 있어야 한다.
...
- 목록은 등록된 리마인더를 세로 카드 리스트로 보여준다.
- 리마인더가 없으면 화면 중앙에 추가 유도 문구를 보여준다.
...
- 캘린더는 보조 탐색 수단
- 홈이 핵심
- 홈, 리마인더 목록, 캘린더, 알림함, 상세 화면의 상태 UI는 공통 톤을 따른다
...
- 홈, 리마인더 목록, 캘린더 선택 날짜, 상세 최근 히스토리 리스트에는 무거운 `AppCard` 톤을 사용하지 않는다
