...
화면 query는 활성 일정 최대 500개와 해당 일정의 전체 처리 기록을 사용자·일정 ID별로 공유하고, 화면 모듈이 표시 범위의 projection을 만든다.
...
상세 화면의 단일 hook이 전체 처리 기록 조회와 대표 상태 계산을 소유하고, 같은 기록에서 실제 처리 시각 최신순 5건을 파생한다.
...
`session.ts`는 요청 병합·종료·OS 쓰기 순서를 관리하고, `sync.ts`는 한 번의 데이터 조회·OS 적용·정리를 수행한다. 조회는 쓰기 큐 밖에서 실행해 로그아웃 정리를 막지 않는다.
`plan.ts`는 후보 선택·현재와 미래 뱃지·예약 차이를 한 번에 계산한다.
`DeviceSyncProvider`는 사용자별 알림 세션을 시작·종료하고 알림과 위젯을 병렬 갱신한다. 요청 병합과 종료 여부는 알림 세션이 소유하며 OS 쓰기 큐만 기기 전체에서 공유한다.
화면과 일정 변경은 `syncDeviceOutputs()`로 두 출력을 명시적으로 갱신한다. 위젯 구현과 데이터 조회는 알림 모듈과 독립적이다.
...
같은 세션에서 실행 중 들어온 알림 동기화 요청은 최신 입력의 후속 실행 하나로 합친다. 각 호출은 자신의 요청 이후 시작한 조회·적용 시도가 끝날 때 완료되며, 같은 세션의 새 요청은 진행 중인 OS 적용을 중단하지 않는다.
